...
setwd("C:/Users/julia/Desktop/3. Semester/QGenetics/Projekt_new/Projekt")
...
threshold_95 <- quantile(positive_data1$IT_SWE_fst1, 0.95, na.rm = TRUE)
threshold_99 <- quantile(positive_data1$IT_SWE_fst1, 0.99, na.rm = TRUE)
...
positive_data1$outlier_95 <- ifelse(positive_data1$IT_SWE_fst1 >  threshold_95, "Outlier", "Non-outlier")
positive_data1$outlier_99 <- ifelse(positive_data1$IT_SWE_fst1 >  threshold_99, "Outlier", "Non-outlier")
...
ifelse(positive_data1$outlier_99 == "Outlier", "Outlier  99%", ifelse(positive_data1$outlier_95 == "Outlier", "Outlier 95%", "Non-outlier"))

ifelse(positive_data1$outlier_95 == "Outlier", "Outlier 95%", "Non-outlier")

"Outlier 99%" = "blue"
# Create a plot with marked outliers, FST mean line, and customized legend
top <- ggplot(positive_data1, aes(mid, IT_SWE_fst1)) +
  geom_point(aes(colour = ifelse(outlier_95 == "Outlier", "Outlier 95%", "Non-outlier")),  size = 2) +
  scale_colour_manual(values = c("Non-outlier" = "black", "Outlier  95%" = "green")) +
  geom_hline(yintercept = mean_fst_Chr1, colour = "yellow") +
  xlab("Position on Chromosome (Mb)") + ylab(expression(italic(F)[ST])) +
  theme_light() +
  ggtitle("Locus Specific Estimate of FST on Chromosome 1") +  # Add  plot title
  guides(color = guide_legend(override.aes = list(size = 3, shape =  16), title = NULL)) +  # Customize legend
...
out_95 <- filter(positive_data1, outlier_95 == "Outlier")

# Filter rows where the 'outlier_99' column has the value "Outlier"
out_99 <- filter(positive_data1, outlier_99 == "Outlier")
...
# Combine the individual boxplots into a single plot
combined_plots <- plot_grid(a_td1, a_td2, a_td3, a_td4, a_td5, labels = "AUTO", nrow = 2)
...
At_data_chr1 <- as.tibble(data.frame(windows1, td1, nd1, fst1, dxy1))
At_data_chr2 <- as.tibble(data.frame(windows2, td2, nd2, fst2, dxy2))
At_data_chr3 <- as.tibble(data.frame(windows3, td3, nd3, fst3, dxy3))
At_data_chr4 <- as.tibble(data.frame(windows4, td4, nd4, fst4, dxy4))
At_data_chr5 <- as.tibble(data.frame(windows5, td5, nd5, fst5, dxy5))

# Means of pi, idk if necessary
At_data_chr1 %>% select(contains("pi")) %>% summarise_all(mean)
At_data_chr2 %>% select(contains("pi")) %>% summarise_all(mean)
At_data_chr3 %>% select(contains("pi")) %>% summarise_all(mean)
At_data_chr4 %>% select(contains("pi")) %>% summarise_all(mean)
At_data_chr5 %>% select(contains("pi")) %>% summarise_all(mean)

# select nucleotide diversity data and calculate means
At_data_chr1 %>% select(contains("td1")) %>% summarise_all(mean)
At_data_chr2 %>% select(contains("td2")) %>% summarise_all(mean)
At_data_chr3 %>% select(contains("td3")) %>% summarise_all(mean)
At_data_chr4 %>% select(contains("td4")) %>% summarise_all(mean)
At_data_chr5 %>% select(contains("td5")) %>% summarise_all(mean)

# To plot this we need to use "gather" on the data
td_g_chr1 <- At_data_chr1 %>% select(contains("td")) %>% gather(key = "populations", value = "td")
td_g_chr2 <- At_data_chr2 %>% select(contains("td")) %>% gather(key = "populations", value = "td")
td_g_chr3 <- At_data_chr3 %>% select(contains("td")) %>% gather(key = "populations", value = "td")
td_g_chr4 <- At_data_chr4 %>% select(contains("td")) %>% gather(key = "populations", value = "td")
td_g_chr5 <- At_data_chr5 %>% select(contains("td")) %>% gather(key = "populations", value = "td")

# Remove rows with missing values
td_g2_chr1 <- na.omit(td_g_chr1)
td_g2_chr2 <- na.omit(td_g_chr2)
td_g2_chr3 <- na.omit(td_g_chr3)
td_g2_chr4 <- na.omit(td_g_chr4)
td_g2_chr5 <- na.omit(td_g_chr5)

td_g2_chr1$log_td <- log10(td_g2_chr1$td)
td_g2_chr2$log_td <- log10(td_g2_chr2$td)
td_g2_chr3$log_td <- log10(td_g2_chr3$td)
td_g2_chr4$log_td <- log10(td_g2_chr4$td)
td_g2_chr5$log_td <- log10(td_g2_chr5$td)


# Make a boxplot using ggplot2
a_td_g2_Chr1 <- ggplot(td_g2_chr1, aes(x = populations, log_td, fill = populations)) +
  geom_boxplot(color = "black") +  # Border color
  scale_fill_manual(values = c("red", "blue","orange","magenta")) +  # Box fill colors, choose number of colours based on number of populations
  xlab(NULL) +
  ylab("Tajima's D") +
  ggtitle("Compare Tajima's D Between Italy and Sweden")

a_td_g2_Chr2 <- ggplot(td_g2_chr2, aes(x = populations, log_td, fill = populations)) +
  geom_boxplot(color = "black") +  # Border color
  scale_fill_manual(values = c("red", "blue","orange","magenta")) +  # Box fill colors, choose number of colours based on number of populations
  xlab(NULL) +
  ylab("Tajima's D") +
...
a_td_g2_Chr3 <- ggplot(td_g2_chr3, aes(x = populations, log_td, fill = populations)) +
  geom_boxplot(color = "black") +  # Border color
  scale_fill_manual(values = c("red", "blue","orange","magenta")) +  # Box fill colors, choose number of colours based on number of populations
  xlab(NULL) +
  ylab("Tajima's D") +
...
a_td_g2_Chr4 <- ggplot(td_g2_chr4, aes(x = populations, log_td, fill = populations)) +
  geom_boxplot(color = "black") +  # Border color
  scale_fill_manual(values = c("red", "blue","orange","magenta")) +  # Box fill colors, choose number of colours based on number of populations
  xlab(NULL) +
  ylab("Tajima's D") +
...
a_td_g2_Chr5 <- ggplot(td_g2_chr5, aes(x = populations, log_td, fill = populations)) +
  geom_boxplot(color = "black") +  # Border color
  scale_fill_manual(values = c("red", "blue","orange","magenta")) +  # Box fill colors, choose number of colours based on number of populations
  xlab(NULL) +
  ylab("Tajima's D") +
...
# Combine the individual boxplots into a single plot
combined_plots <- plot_grid(a_td_g2_Chr1, a_td_g2_Chr2, a_td_g2_Chr3, a_td_g2_Chr4, a_td_g2_Chr5, labels = "AUTO", nrow = 2)
...
subset_IT_td <- log(At_data_chr1$IT_td[!is.na(At_data_chr1$IT_td) & !is.nan(log(At_data_chr1$IT_td))])
subset_SW_td <- log(At_data_chr1$SWE_td[!is.na(At_data_chr1$SWE_td) & !is.nan(log(At_data_chr1$SWE_td))])
...
plot(density(subset_SW_td), col = "blue", main = "Distribution log TajimaD", xlim = c(-10,-2))

# Add density plots for other populations with different colors
lines(density(subset_IT_td), col="red")
#lines(density(subset_GER_td), col="orange")


# Add legend
legend("topright", legend=c("SW", "IT"),
       col=c("blue", "red", "orange"), lty=1,
       title="Population")

### Chr4

subset_IT_td4 <- log(At_data_chr4$IT_td[!is.na(At_data_chr4$IT_td) & !is.nan(log(At_data_chr4$IT_td))])
subset_SW_td4 <- log(At_data_chr4$SWE_td[!is.na(At_data_chr4$SWE_td) & !is.nan(log(At_data_chr4$SWE_td))])

# modify code change xlim to see complete distribution
plot(density(subset_SW_td4), col = "blue", main = "Distribution log TajimaD of Chr. 4", xlim = c(-10,-2))

# Add density plots for other populations with different colors
lines(density(subset_IT_td4), col="red")
#lines(density(subset_GER_td), col="orange")
...
subset_IT_td5 <- log(At_data_chr5$IT_td[!is.na(At_data_chr5$IT_td) & !is.nan(log(At_data_chr5$IT_td))])
subset_SW_td5 <- log(At_data_chr5$SWE_td[!is.na(At_data_chr5$SWE_td) & !is.nan(log(At_data_chr5$SWE_td))])

# modify code change xlim to see complete distribution
plot(density(subset_IT_td5), col = "blue", main = "Distribution log TajimaD of Chr. 5", xlim = c(-10,-2))

# Add density plots for other populations with different colors
lines(density(subset_SW_td5), col="red")
#lines(density(subset_GER_td), col="orange")
...
gff <- read.table("/Users/julia/Desktop/3. Semester/QGenetics/TAIR10_GFF3_genes.gff", sep="\t", quote="")

colnames(gff) <- c("chromosome", "source", "type", "start", "end", "score", "strand", "phase", "attributes")
...
# Assuming 'chr' column exists or is known; replace 'chr1' with actual chromosome data if available
gene_ranges <- GRanges(
...
# Create a GRanges object for the outlier regions, using the 'chromosome' column for seqnames
outlier_ranges <- GRanges(
  seqnames = as.factor(all_outs$chromosome),
  ranges = IRanges(start = all_outs$start, end = all_outs$stop)
)


overlaps <- findOverlaps(outlier_ranges, gene_ranges)

# Extract indices of overlapping genes
overlapping_genes_indices <- queryHits(overlaps)

# Extract information for overlapping genes
overlapping_genes_info <- filtered_annotation[overlapping_genes_indices, ]
